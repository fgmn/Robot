...
def Hough_circle(imgGray, canvas):
    ...
    img = cv2.medianBlur(imgGray, 3)        ## 中值、高斯滤波
...
while True:
    ...
    # 坐标（图像内的）
    cv2.putText(img, "({:0<2d}, {:0<2d}, {:0<2f})".format(targetPos_x, targetPos_y, R_ball), (20, 30),   ## 取2位左对齐
                cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 2)  # 文字
    draw_direction(img, lastPos_x, lastPos_y, targetPos_x, targetPos_y)     ## 指示球的方向变化

    # 霍夫圆检测Start
    Hough_circle(r_minus_b, imgHough)   ## imgHough 作为 canvas
    ...
    ## 输出图像参数
    # print(imgStack.shape)     (480, 1280, 3)
